...
public class EntityMetadata {
    ...
    /**
     * Resolves the ID generation strategy for the ID field.
     * Checks for MyBatis-Plus @TableId(type) or JPA @GeneratedValue(strategy) annotations.
     * 
     * @param field the ID field
     * @return the detected strategy, defaults to AUTO if not specified
     */
    private IdStrategy resolveIdStrategy(Field field) {
        // Check MyBatis-Plus @TableId(type = IdType.*)
        String mpIdType = getAnnotationValue(field, "com.baomidou.mybatisplus.annotation.TableId", "type");
        if (mpIdType != null && !mpIdType.isEmpty()) {
            // MyBatis-Plus IdType enum values: AUTO, NONE, INPUT, ASSIGN_ID, ASSIGN_UUID
            if (mpIdType.contains("INPUT") || mpIdType.contains("NONE")) {
                return IdStrategy.INPUT;
            } else if (mpIdType.contains("ASSIGN_UUID")) {
                return IdStrategy.ASSIGN_UUID;
            } else if (mpIdType.contains("ASSIGN_ID")) {
                return IdStrategy.ASSIGN_ID;
            } else if (mpIdType.contains("AUTO")) {
                return IdStrategy.AUTO;
            }
        }
        
        // Check JPA @GeneratedValue(strategy = GenerationType.*)
        String jpaStrategy = getAnnotationValue(field, "javax.persistence.GeneratedValue", "strategy");
        if (jpaStrategy == null) {
            jpaStrategy = getAnnotationValue(field, "jakarta.persistence.GeneratedValue", "strategy");
        }
        if (jpaStrategy != null && !jpaStrategy.isEmpty()) {
            // JPA GenerationType: TABLE, SEQUENCE, IDENTITY, UUID, AUTO
            if (jpaStrategy.contains("UUID")) {
                return IdStrategy.ASSIGN_UUID;
            } else if (jpaStrategy.contains("IDENTITY") || jpaStrategy.contains("AUTO") 
                    || jpaStrategy.contains("SEQUENCE") || jpaStrategy.contains("TABLE")) {
                return IdStrategy.AUTO;
            }
        }
        
        // Check if JPA @GeneratedValue annotation exists (any strategy means AUTO)
        if (hasAnnotation(field, "javax.persistence.GeneratedValue") ||
            hasAnnotation(field, "jakarta.persistence.GeneratedValue")) {
            return IdStrategy.AUTO;
        }
        
        // If field type is String and no auto-generation annotation, assume INPUT
        if (field.getType() == String.class) {
            return IdStrategy.INPUT;
        }
        
        // Default to AUTO for numeric types
        return IdStrategy.AUTO;
    }
    ...
    /**
     * Gets the ID generation strategy for this entity.
     * 
     * @return the ID strategy (AUTO, INPUT, ASSIGN_ID, or ASSIGN_UUID)
     */
    ...
    /**
     * Returns true if the ID should be generated by the database.
     * This is the case for AUTO strategy.
     */
    public boolean isDatabaseGeneratedId() {
        return idStrategy == IdStrategy.AUTO;
    }
    
    /**
     * Returns true if the application provides the ID value.
     * This is the case for INPUT, ASSIGN_ID, and ASSIGN_UUID strategies.
     */
    public boolean isApplicationProvidedId() {
        return idStrategy == IdStrategy.INPUT 
            || idStrategy == IdStrategy.ASSIGN_ID 
            || idStrategy == IdStrategy.ASSIGN_UUID;
    }
