...
/**
 * Copyable reference for Spring Boot + TestNG with:
 * - a single-table fixture on one datasource
 * - a sharded dynamic table on another datasource
 * - act-only data creation into the sharded table
 * - the latest FlowTest v2 DSL: watch + verify + run
 */
@SpringBootTest(
    classes = FlowTestV2ShardedDynamicTableReferenceSpringBootTestNgExampleTest.TestApplication.class,
    properties = {
...
public class FlowTestV2ShardedDynamicTableReferenceSpringBootTestNgExampleTest
    ...
    @Test
    public void shouldCreateShardedDynamicOrderFromSingleTableFixture() throws Exception {
        FixtureHandle<UserProfile> user = FixtureHandle.named(UserProfile.class, "user");

        FlowTestV2.scenario("spring-boot-testng-single-table-fixture-and-sharded-dynamic-table")
            ...
            .when(() -> tradeOrderService.createTradeOrder("a", 100L, 1L, 901L))
            .verify(ctx -> {
                ctx.success();
                assertThat(ctx.result()).isEqualTo(901L);

                assertThat(ctx.fixture(user).before().getName()).isEqualTo("Alice");
                ...
                assertThat(ctx.table("ft_trade_order").insertedCount()).isEqualTo(1L);
                assertThat(ctx.table("ft_trade_order").insertedOne().getColumn("id")).isEqualTo(901L);
                assertThat(ctx.table("ft_trade_order").insertedOne().getColumn("tenant_id")).isEqualTo(100L);
                assertThat(ctx.table("ft_trade_order").insertedOne().getColumn("user_id")).isEqualTo(1L);
                assertThat(ctx.table("ft_trade_order").insertedOne().getColumn("status")).isEqualTo("CREATED");
            })
            .run();
    ...
    static class TestApplication {
        ...
        public DataSource orderDataSource() {
            ...
            dataSource.setURL("jdbc:h2:mem:flowtest_v2_testng_reference_order_ds;MODE=MYSQL;DB_CLOSE_DELAY=-1");
        ...
        public DataSource accountDataSource() {
            JdbcDataSource dataSource = new JdbcDataSource();
            dataSource.setURL("jdbc:h2:mem:flowtest_v2_testng_reference_account_ds;MODE=MYSQL;DB_CLOSE_DELAY=-1");
            dataSource.setUser("sa");
            dataSource.setPassword("");
            return dataSource;
        }
